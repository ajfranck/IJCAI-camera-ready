...
\begin{itemize}
    ...
    \item Within the training parameter range, the surrogate holds sub-$0.1\%$ relative $L^2$ error on both fields while running nearly three orders of magnitude faster than the finite-difference baseline. This enables near real-time FHN simulation for downstream neuroscience workloads such as inverse parameter inference and closed-loop stimulation.
...
\end{equation}
classifies the local flow. The regime is excitable when there is a single fixed point with $\mathrm{tr}\,J < 0$ and $\det J > 0$ (which is a stable rest state), oscillatory when there is a single fixed point with $\mathrm{tr}\,J > 0$, which is an unstable focus surrounded by a limit cycle. It is bistable when there are three real fixed points.

\Cref{fig:bifurcation} maps these regimes on the $(a, b)$ plane at the median training $\tau$ and overlays the two parameter boxes the paper models. The first surrogate's box (\Cref{sec:data_generation}) clearly sits in the oscillatory region: a finer sweep over $\tau \in [1, 20]$ confirms $100\%$ of the sampled cube is oscillatory, so the surrogate is trained specifically on limit-cycle neurons firing tonically rather than a dormant rest state. These same criteria place the box of the excitable surrogate (\Cref{sec:excitable_results}, $|u^\star| > 1$) entirely in the stable-rest region. Thus, a single analysis defines both dynamical regimes the paper models.
...
\noindent\textbf{1. Lifting:} An initial projection $\mathcal{P}: \mathbb{R}^{d_{\text{in}}} \rightarrow \mathbb{R}^w$ maps the input channels (here $d_{\text{in}} = 2$ for $(u,v)$) to a higher-dimensional representation of width $w$:
\begin{equation}
    \bm{z}_0(\bm{x}) = \mathcal{P}(\bm{u}_0(\bm{x})).
\end{equation}

\noindent\textbf{2. Fourier Layers:} The core of the FNO consists of $L$ Fourier layers. Each layer $\ell \in \{1, \ldots, L\}$ applies a convolution and then a nonlinearity:
\begin{equation}
...
\noindent\textbf{3. Projection:} A projection $\mathcal{Q}: \mathbb{R}^w \rightarrow \mathbb{R}^{d_{\text{out}}}$ maps from the hidden dimension back to the output space ($d_{\text{out}} = 2$ for $(u,v)$):
\begin{equation}
...
Our analysis proceeds in stages. We first assess single-step prediction accuracy on held-out validation data, then the error correlation for each parameter, then long-time autoregressive rollout performance, and then the computational efficiency gains relative to traditional finite-difference solvers. We finally turn to generalization beyond the training ranges and to a complementary excitable regime in which the operator reproduces excitable-media phenomena. Unless noted otherwise, results are reported on the validation set consisting of 1,600 trajectories with parameter combinations and initial conditions not seen during training.

\subsection{Single-Step Prediction Accuracy}
\label{sec:single_step_results}

We begin by evaluating the FNO's ability to predict single time steps. Table~\ref{tab:single_step_metrics} summarizes the quantitative performance metrics averaged over the entire validation set.
...
\end{small}
\smallskip
\caption{Single-step prediction accuracy on the held-out validation set ($N_{\text{test}} = 1600$ trajectories, $n_x = 256$ spatial points), in normalized space. Values are 5-seed means (per-trajectory mean for relative $L^2$, element-wise averages otherwise). Per-seed variability is reported in \Cref{tab:baselines}.}
...
\end{center}
\smallskip
\caption{Baseline comparison on the validation set ($N_{\text{test}} = 1600$
...
\noindent\textbf{Baseline comparison.} Table~\ref{tab:baselines} compares two natural baselines. DeepONet is far worse: its relative $L^2$ error is roughly $58\times$ higher on $u$ and $50\times$ higher on $v$. The channel-broadcast FNO is more competitive but still performs worse than the FiLM-conditioned model by roughly $30\%$ on both fields, while still containing $26\%$ more parameters ($0.63$M vs $0.50$M). FiLM conditioning therefore improves accuracy and reduces model size relative to broadcasting $\bm{\lambda}$ as constant channels, so the gain comes from how parameters are added rather than from added capacity.

Qualitative single-step predictions across weakly, moderately, and strongly diffusive regimes are visually indistinguishable from ground truth (\Cref{fig:single_step_comparison}). The per-regime single-step MAE ranges from $5 \times 10^{-5}$ to $2 \times 10^{-4}$, with the weakly diffusive case being the hardest.

\begin{figure}[!ht]
\centering
\includegraphics[width=\columnwidth]{figures/single_step_regimes.png}
\caption{Single-step prediction across three representative validation regimes (rows: weakly, moderately, strongly diffusive). Left: ground truth vs predicted $u$. Center: ground truth vs predicted $v$. Right: absolute error in $u$ and $v$ (scaled for visibility). Predicted and true profiles are visually indistinguishable.}
...
\midrule
Within range (validation) & $0.0006655$ & $0.0008012$ \\
...
\paragraph{Outlook: conduction-block design.}
\label{sec:outlook}
A future goal is signal-based conduction blocking: finding a stimulus $I_{\text{ext}}(\bm{x}, t)$ that halts action-potential propagation, the basis of kHz nerve block~\cite{Kilgore2014}. Because the FNO is differentiable in $\bm{\lambda}$ and its inputs, these signals are a gradient-descent target. Differentiating throughout the rollout, we obtain the search direction in one backward pass, while the numerical solver requires a separate integration for every possible solution. Since the FiLM conditioning already spans the parameter family, a candidate signal can also be stress-tested for robustness across $\bm{\lambda}$, moving our surrogate toward equation-free and system-level neuromodulation design~\cite{fabiani2025enabling}.
